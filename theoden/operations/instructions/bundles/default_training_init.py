...
from .. import ClosedDistribution, InitGlobalModelAction, Initializer, OpenDistribution
...
class DefaultTrainingInitInstructionBundle(InstructionBundle, Transferable):
    def __init__(
        ...
        global_balancing_function: (
            BalancingDistribution | dict[str, BalancingDistribution] | None
        ) = None,
...
class DefaultTrainingInitOpenDistribution(OpenDistribution):
    def __init__(
        self,
        dataset: SampleDataset | dict[str, SampleDataset],
        losses: list[Loss],
        ...
        # create the commands for the sequential command
        commands = [
            *(pre_commands if pre_commands else []),
            LoadDatasetCommand(dataset=dataset),
            *([InitModelCommand(model=model, model_key=model_key)] if model else []),
            SetLossesCommand(losses=losses),
            SetOptimizerCommand(optimizer=optimizer, model_key=model_key),
        ]
        ...
        commands.extend(post_commands if post_commands else [])
